...
export const ConsolidatedSettingsProvider: React.FC<ConsolidatedSettingsProviderProps> = ({ children }) => {
  ...
  const loadSettings = useCallback(async (): Promise<void> => {
    try {
      const storageKey = user?.id ? `settings_${user.id}` : 'settings_global';
      ...
      if (storedSettings) {
        ...
        if (parsedSettings.preferences?.completedFeatures) {
          parsedSettings.preferences.completedFeatures = new Set(parsedSettings.preferences.completedFeatures);
        }
        if (parsedSettings.preferences?.dismissedHelp) {
          parsedSettings.preferences.dismissedHelp = new Set(parsedSettings.preferences.dismissedHelp);
        }
    ...
    try {
      ...
      const settingsToSave = {
        ...
        preferences: {
          ...
          completedFeatures: Array.from(newSettings.preferences.completedFeatures),
          dismissedHelp: Array.from(newSettings.preferences.dismissedHelp),
        },
      };
