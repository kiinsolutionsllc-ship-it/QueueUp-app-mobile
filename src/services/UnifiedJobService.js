...
class UnifiedJobService {
  ...
  async loadData() {
    ...
    try {
      ...
      const [jobsData, bidsData, messagesData, jobHistoryData] = await Promise.all([
        AsyncStorage.getItem('unified_jobs'),
        AsyncStorage.getItem('unified_bids'),
        AsyncStorage.getItem('unified_messages'),
        AsyncStorage.getItem('unified_job_history')
      ]);

      // Parse and set data if it exists
      this.jobs = jobsData ? JSON.parse(jobsData) : [];
      this.bids = bidsData ? JSON.parse(bidsData) : [];
  ...
  async saveData() {
    try {
      await Promise.all([
        AsyncStorage.setItem('unified_jobs', JSON.stringify(this.jobs)),
        AsyncStorage.setItem('unified_bids', JSON.stringify(this.bids)),
  ...
  async clearAllData() {
    try {
      await Promise.all([
        AsyncStorage.removeItem('unified_jobs'),
        AsyncStorage.removeItem('unified_bids'),
        AsyncStorage.removeItem('unified_messages'),
        AsyncStorage.removeItem('unified_job_history'),
        AsyncStorage.removeItem('jobs'),
        AsyncStorage.removeItem('bids'),
        AsyncStorage.removeItem('messages'),
        AsyncStorage.removeItem('jobHistory')
      ]);
      ...
      console.log('UnifiedJobService: All data cleared for testing');
    } catch (error) {
  ...
  async createJob(jobData) {
    ...
    const job = {
      id: uniqueIdGenerator.generateJobId(),
      ...jobData,
      status: 'posted',
  ...
  async resetAllData() {
    try {
      ...
      // Clear AsyncStorage - Core job data (both new and old keys)
      console.log('UnifiedJobService: Clearing AsyncStorage...');
      await Promise.all([
        AsyncStorage.removeItem('unified_jobs'),
        AsyncStorage.removeItem('unified_bids'),
        AsyncStorage.removeItem('unified_messages'),
      ...
      console.log('UnifiedJobService: Cleared core AsyncStorage keys');
      
      // Clear AsyncStorage - Messaging data
      await AsyncStorage.removeItem('conversations');
      await AsyncStorage.removeItem('messages');
      await AsyncStorage.removeItem('unified_messaging_conversations');
      await AsyncStorage.removeItem('unified_messaging_messages');
      await AsyncStorage.removeItem('unified_messaging_users');
      await AsyncStorage.removeItem('enhanced_messaging_conversations');
      await AsyncStorage.removeItem('enhanced_messaging_messages');
      await AsyncStorage.removeItem('enhanced_messaging_users');
