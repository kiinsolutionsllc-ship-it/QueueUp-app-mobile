...
class SubscriptionService {
  ...
  async loadData() {
    try {
      const [plansData, subscriptionsData, paymentsData, usageData, trialHistoryData, abusePreventionData] = await Promise.all([
        AsyncStorage.getItem(this.PLANS_KEY),
        AsyncStorage.getItem(this.SUBSCRIPTIONS_KEY),
        AsyncStorage.getItem(this.PAYMENTS_KEY),
        AsyncStorage.getItem(this.USAGE_KEY),
        AsyncStorage.getItem(this.TRIAL_HISTORY_KEY),
        AsyncStorage.getItem(this.ABUSE_PREVENTION_KEY)
      ]);

      this.subscriptionPlans = plansData ? JSON.parse(plansData) : [];
      this.userSubscriptions = subscriptionsData ? JSON.parse(subscriptionsData) : [];
      this.subscriptionPayments = paymentsData ? JSON.parse(paymentsData) : [];
      this.subscriptionUsage = usageData ? JSON.parse(usageData) : [];
      this.trialHistory = trialHistoryData ? JSON.parse(trialHistoryData) : [];
      this.abusePrevention = abusePreventionData ? JSON.parse(abusePreventionData) : {
        emailTrialCount: {},
        deviceFingerprints: {},
        ipAddresses: {},
        paymentMethods: {},
  ...
  async saveData() {
    try {
      await Promise.all([
        AsyncStorage.setItem(this.PLANS_KEY, JSON.stringify(this.subscriptionPlans)),
        AsyncStorage.setItem(this.SUBSCRIPTIONS_KEY, JSON.stringify(this.userSubscriptions)),
        AsyncStorage.setItem(this.PAYMENTS_KEY, JSON.stringify(this.subscriptionPayments)),
        AsyncStorage.setItem(this.USAGE_KEY, JSON.stringify(this.subscriptionUsage)),
        AsyncStorage.setItem(this.TRIAL_HISTORY_KEY, JSON.stringify(this.trialHistory)),
        AsyncStorage.setItem(this.ABUSE_PREVENTION_KEY, JSON.stringify(this.abusePrevention))
      ]);
    } catch (error) {
