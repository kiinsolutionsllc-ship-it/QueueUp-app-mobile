...
class EnhancedUnifiedMessagingService {
      ...
      if (usersData) {
        try {
          ...
          if (Array.isArray(usersArray) && usersArray.length > 0) {
            if (Array.isArray(usersArray[0]) && usersArray[0].length === 2) {
              this.users = new Map(usersArray);
            } else {
              console.warn('EnhancedUnifiedMessagingService: Converting old user format to Map');
              this.users = new Map();
          ...
          console.error('EnhancedUnifiedMessagingService: Error parsing users data:', error);
          this.users = new Map();
        }
      }
      
    } catch (error) {
      console.error('EnhancedUnifiedMessagingService: Error loading data:', error);
    }
  }
  ...
  async saveData() {
    try {
      ...
      await Promise.all([
        AsyncStorage.setItem(this.CONVERSATIONS_KEY, JSON.stringify(this.conversations)),
        AsyncStorage.setItem(this.MESSAGES_KEY, JSON.stringify(this.messages)),
        AsyncStorage.setItem(this.USERS_KEY, JSON.stringify(usersArray))
      ]);
    } catch (error) {
      console.error('EnhancedUnifiedMessagingService: Error saving data:', error);
    }
  }
