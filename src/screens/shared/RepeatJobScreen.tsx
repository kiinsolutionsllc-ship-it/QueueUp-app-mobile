...
interface RepeatJobScreenProps {
  navigation: any;
  route: {
    params?: any;
  };
}
export default function RepeatJobScreen({ navigation, route }: RepeatJobScreenProps) {
  ...
  const { originalJob } = route.params || {};
  ...
  const validationRules = {
    modifications: [{ validator: 'maxLength', message: 'Modifications must be less than 500 characters' }],
  };

  const handleRepeatJob = async (values: any) => {
    if (!selectedDate || !selectedTime) {
      Alert.alert('Selection Required', 'Please select both date and time for the repeated service');
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        'Job Repeated Successfully!',
        `The service has been scheduled for ${selectedDate} at ${selectedTime}. The customer will be notified.`,
        [
          {
            text: 'OK',
      ...
      Alert.alert('Error', 'Failed to repeat job. Please try again.');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        >
              ...
              <View style={styles.jobDetails}>
                <View style={styles.jobDetail}>
                  ...
                  <Text style={[styles.jobDetailText, { color: theme.textSecondary }]}>
                    {originalJob.customerName || 'Customer'}
                  </Text>
                </View>
                <View style={styles.jobDetail}>
          ...
          <View style={styles.section}>
            ...
            <Text style={[styles.sectionSubtitle, { color: theme.textSecondary }]}>
              Choose when you'd like to repeat this service
            </Text>
          ...
          <ValidatedForm
            initialValues={{ modifications: '' }}
            validationRules={validationRules}
            onSubmit={handleRepeatJob}
            submitButtonText="Repeat Job"
          ...
          >
            {({ MaterialTextInput }: any) => (
              <View style={styles.section}>
                ...
                <Text style={[styles.sectionSubtitle, { color: theme.textSecondary }]}>
                  Note any changes or additional requirements for this repeat service
                </Text>
                
                <MaterialTextInput
                  name="modifications"
                  label="Modifications (Optional)"
                  placeholder="Any changes or additional requirements..."
                  multiline
                  numberOfLines={3}
                  leftIcon="edit"
                />
          ...
          <MaterialCard style={[styles.infoCard, { backgroundColor: theme.surface }]}>
            ...
            <Text style={[styles.infoText, { color: theme.textSecondary }]}>
              • The customer will be notified of the repeated service
            </Text>
            <Text style={[styles.infoText, { color: theme.textSecondary }]}>
              • Same pricing and service terms apply unless modified
            </Text>
            <Text style={[styles.infoText, { color: theme.textSecondary }]}>
              • You can make changes to the service description if needed
            </Text>
            <Text style={[styles.infoText, { color: theme.textSecondary }]}>
              • The job will appear in your active jobs list once confirmed
            </Text>
          </MaterialCard>
        </ScrollView>
      </KeyboardAvoidingView>
