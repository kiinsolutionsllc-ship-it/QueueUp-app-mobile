...
const BidComparisonScreen = ({ navigation, route }) => {
  ...
  const fetchJobsWithBids = useCallback(async () => {
    try {
      ...
      const formattedJobs = jobsThatCanReceiveBids.map(job => {
        const jobBids = bids.filter(bid => bid.jobId === job.id && bid.status === 'pending');
  ...
  const handleMessageMechanic = (bid) => {
    hapticService.light();
    try {
  ...
  const processBidAcceptance = async (bid) => {
    try {
  ...
  const renderBidCard = (bid, allBids) => {
    const isSelected = selectedBid?.id === bid.id;
    const isLowestBid = bid.amount === Math.min(...allBids.map(b => b.amount));
      ...
      >
        ...
        <View style={styles.mechanicSpecialties}>
          ...
          <View style={styles.specialtiesList}>
            {bid.mechanic.specialties.map((specialty, index) => (
              <View
                key={index}
  ...
  const renderJobAccordion = (job) => {
    const isExpanded = expandedJobs.has(job.id);
    const isSelected = selectedJobId === job.id;
    const lowestBid = job.bids.length > 0 ? Math.min(...job.bids.map(b => b.amount)) : 0;
    const highestBid = job.bids.length > 0 ? Math.max(...job.bids.map(b => b.amount)) : 0;
