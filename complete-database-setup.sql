-- Complete QueueUp Database Setup
...
-- Create RLS policies (including the missing INSERT policy for users)
-- Users policies
...
-- Bids policies
CREATE POLICY "Mechanics can create bids" ON bids FOR INSERT WITH CHECK (auth.uid() = mechanic_id);
CREATE POLICY "Mechanics can update own bids" ON bids FOR UPDATE USING (auth.uid() = mechanic_id);
CREATE POLICY "Users can view job bids" ON bids FOR SELECT USING (
  EXISTS (SELECT 1 FROM jobs WHERE jobs.id = bids.job_id AND (jobs.customer_id = auth.uid() OR jobs.mechanic_id = auth.uid()))
);
...
CREATE POLICY "Users can view own conversations" ON conversations FOR SELECT USING (auth.uid() = customer_id OR auth.uid() = mechanic_id);
CREATE POLICY "Users can create conversations" ON conversations FOR INSERT WITH CHECK (auth.uid() = customer_id OR auth.uid() = mechanic_id);
CREATE POLICY "Users can update own conversations" ON conversations FOR UPDATE USING (auth.uid() = customer_id OR auth.uid() = mechanic_id);

-- Reviews policies
CREATE POLICY "Users can view job reviews" ON reviews FOR SELECT USING (
  EXISTS (SELECT 1 FROM jobs WHERE jobs.id = reviews.job_id AND (jobs.customer_id = auth.uid() OR jobs.mechanic_id = auth.uid()))
);
...
DO $$
BEGIN
    RAISE NOTICE 'QueueUp database setup completed successfully! All tables, indexes, triggers, and RLS policies are now in place.';
END $$;
